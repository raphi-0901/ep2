...
public class LayeredRasterRGBA implements RasterizedRGBIterable {
    ...
    public LayeredRasterRGBA(int width, int height) {
        this.width = width;
        this.height = height;
        this.layers = new LinkedListRasterRGBA();
        setActiveLayer(0);
        layers.addFirst(newLayerFilledWithColor(new Color(0, 0, 0, 255)));
    }

    private RasterRGBA newLayerFilledWithColor(Color color) {
        RasterRGBA raster = new RasterRGBA(this.getWidth(), this.getHeight());
    ...
    public void newLayer() {
        // fill all pixel to transparent
        layers.addFirst(newLayerFilledWithColor(new Color(0, 0, 0, 0)));
        setActiveLayer(0);
    }
    ...
    public Color getPixelColor(int x, int y) {
        Color color = this.layers.get(0).getPixelColor(x,y);
        for (int i = 1; i < numberOfLayers(); i++) {
            color = RasterRGBA.over(this.layers.get(i).getPixelColor(x,y), color);
        }
    ...
    public void setPixelColor(int x, int y, Color color) {
        this.layers.get(activeLayer).setPixelColor(x, y, color);
    }
    ...
    public RasterRGBA asRasterRGBA() {
        RasterRGBA layer = newLayerFilledWithColor(new Color(0, 0, 0, 0));
        for (int i = 0; i < numberOfLayers(); i++) {
            layer = this.layers.get(i).over(layer);
        }
    ...
    public void crop(int width, int height) {
        ...
        for (int i = 0; i < this.layers.size(); i++) {
            this.layers.get(i).crop(width, height);
        }
    }
    ...
    public void drawLine(int x1, int y1, int x2, int y2, Color color) {
        this.layers.get(activeLayer).drawLine(x1, y1, x2, y2, color);
    }

    @Override
    public RasterizedRGBIterator iterator() {
        return new MyLayeredRasterRGBAIter(layers);
    }
}
